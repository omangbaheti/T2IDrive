...
// {
//     public class HPUIInteractableCanvas : MonoBehaviour
//     {
//         public UnityEvent<HPUIGestureEventArgs, HPUICanvasEventArgs> OnCanvasInteractions = new();
//         public Dictionary<Vector2Int, Collider> coordsToCollider = new Dictionary<Vector2Int, Collider>();
...
//
//         [SerializeField] private SerializedDictionary<Vector2Int, HPUIMeshContinuousInteractable> HPUICanvasComponents = new();
//
//         [Tooltip("In Percent")]
...
//         [SerializeField, HideInInspector] private SerializedDictionary<HPUIMeshContinuousInteractable, Vector2Int> canvasToID = new();
//
...
//         {
//             foreach ((Vector2Int ID, HPUIMeshContinuousInteractable canvas) in HPUICanvasComponents)
//             {
//                 canvas.GestureEvent.AddListener(HandleGesture);
//                 canvasToID.Add(canvas, ID);
//             }
...
//             foreach ((Vector2Int ID, HPUIMeshContinuousInteractable canvas) in HPUICanvasComponents)
//             {
//                 canvas.GestureEvent.RemoveListener(HandleGesture);
//             }
//             posFilter = new(90, posFilterMinCutoff, posFilterBeta);
//             canvasToID.Clear();
//         }
...
//         {
//             int maxX = 0, maxY = 0;
//             foreach ((Vector2Int ID, HPUIMeshContinuousInteractable interactable) in HPUICanvasComponents)
//             {
//                 maxX = Math.Max(maxX, ID.x);
//                 maxY = Math.Max(maxY, ID.y);
//             }
//
//             HPUIMeshContinuousInteractable[][] components = new HPUIMeshContinuousInteractable[maxX+1][];
//             for (int i = 0; i <= maxX; i++)
//             {
//                 components[i] = new HPUIMeshContinuousInteractable[maxY+1]; // Initialize inner arrays
//             }
//
//             foreach ((Vector2Int ID, HPUIMeshContinuousInteractable interactable) in HPUICanvasComponents)
//             {
//                 components[ID.x][ID.y] = interactable;
//             }
//
//             for (int i = 0; i < components.Length; i++)
//             {
//                 x_size += components[i][0].X_size;
//                 meshXResolution += components[i][0].MeshXResolution;
//             }
//
//             for (int j = 0; j < components[0].Length; j++)
//             {
//                 y_size += components[0][j].Y_size;
//                 meshYResolution += components[0][j].MeshYResolution;
//             }
//
//             foreach ((Vector2Int ID, HPUIMeshContinuousInteractable interactable) in HPUICanvasComponents)
//             {
//                 foreach ((Vector2Int key, Collider value) in interactable.ContinuousCollidersManager.RawCoordsToCollider)
//                 {
//                     Vector2Int coordinate = new Vector2Int(key.x + ID.x * interactable.MeshXResolution, key.y + ID.y * interactable.MeshYResolution);
//                     coordsToCollider[coordinate] = value;
//                 }
//             }
//
//             minBounds = Vector2.zero;
//             maxBounds = new Vector2(components.Length, components[0].Length);
//         }
...
//                     if (currentGesturePoints.Count < 5 || eventArgs.CumulativeDistance < 0.001f)
//                     {
...
//             if (canvasToID.TryGetValue(canvasComponent, out Vector2Int id))
//             {
//                 processedTouchPos.x = Mathf.Clamp(id.x + processedTouchPos.x, 0, maxBounds.x);
//                 processedTouchPos.y = Mathf.Clamp(id.y + processedTouchPos.y, 0, maxBounds.y);
//                 processedPosition = applyPosFilter ? posFilter.Filter(processedTouchPos) : processedTouchPos;
//                 return true;
//             }
//             else
//             {
//                 Debug.LogError($"Unregistered Canvas: {canvasComponent.transform.name}");
...
//         {
//             Vector2 canvasMinBounds = canvasToID[currentInteractable];
