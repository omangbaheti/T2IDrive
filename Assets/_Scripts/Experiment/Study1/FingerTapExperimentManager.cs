...
{
    public class FingerTapExperimentManager : ExperimentManager<SwipeGestureBlockData>
    {
        [SerializeField] private TextMeshProUGUI currentTrialText;
        ...
        private List<FingerRegions> fingerRegions = new()
        {
            FingerRegions.RadialDistal,
            FingerRegions.RadialIntermediate,
            FingerRegions.RadialProximal,
            FingerRegions.VolarDistal,
            FingerRegions.VolarIntermediate,
            FingerRegions.VolarProximal
        };
        protected override void OnSessionBegin(Session session)
        {
            ...
            rng = new(int.Parse(Session.instance.ppid));
        ...
        protected override void ConfigureBlock(SwipeGestureBlockData el, Block block, bool lastBlockCancelled)
        {
            ...
            List<FingerRegions> shuffledRegions = ShuffleList(fingerRegions);

            foreach (FingerRegions region in shuffledRegions)
            {
                Trial trial = block.CreateTrial();
                trial.settings.SetValue(StudyLogs.StartRegion, region);
                trial.settings.SetValue(StudyLogs.EndRegion, region);
                Debug.Log($"Create Trial {trial.number} with Target Region {region}");
            }
        ...
        {
            Dictionary<string, string> regionToInt = new()
            {
                { "RadialDistal", "1" },
            ...
            string currentTrial = trial.settings.GetString(StudyLogs.StartRegion);
            currentTrialText.text = $"{regionToInt[currentTrial]}";
            trialManagers[currentFinger].SetCurrentTrialActive(trial);
        ...
        private void HandleTrial(HPUICanvasEventArgs args)
        {
            ...
            else
            {
                ...
                if (args.State == HPUICanvasState.Completed) NextTrial();
                else CancelTrial();
            }
